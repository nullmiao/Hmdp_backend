...
/// ///redis是单线程的，所以多个线程执行lua脚本就不会出现并发安全问题
...
@Transactional
    ...
    Integer count = query().eq("user_id", userId).eq("voucher_id", take).count();
        if (count > 0) {
            log.error("买过一次");
        }
        boolean success = seckillVoucherService.update().setSql("stock = stock - 1")
